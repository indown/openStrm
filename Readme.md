...
- 支持自定义前缀（方便配合媒体服务器使用）
- 基于115目录树生成
- 支持夸克网盘账号：同步生成 strm、随片下载字幕 / nfo。夸克没有整树导出，一次同步要逐个目录翻页，大库别把定时设太密；夸克直链取文件必须带 Cookie，所以不支持 302
...
- **直接发链接**：磁力 / ed2k / http 链接 → 选下到哪个任务目录（可逐层进入子文件夹，比如 `tv/某剧/Season 1`），交给 115 云下载，下完自动生成 strm；配置了「复制到 OpenList」的话还会多一个「115 默认目录，下完让 OpenList 复制走」按钮（同样能逐层选复制到哪个子文件夹）；115 分享链接 → 看一眼内容，选转存到哪个任务目录（同样可进子文件夹）并触发同步。
