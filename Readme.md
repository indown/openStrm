...
- 智能体接入（MCP）：Claude Code、Codex、Open WebUI 这类 AI 客户端用令牌连上来，claude.ai、ChatGPT 网页从公网走 OAuth 连上来，能查任务和同步进度、开始同步、看分享、转存、添加云下载，替你核对整理清单（认不准的去 TMDB 找、冲突选办法）、你点头后执行或撤销，管理追更，查找、体检、修正 strm；分权限档，每次调用都有记录
...
- **权限档**：只读（查任务、同步进度和记录、网盘目录、分享内容、云下载列表、整理清单、追更、strm 体检）/ 日常（再加开始 / 取消同步、转存分享、添加云下载、整理的预览 / 改清单 / 执行 / 撤销、追更的检查和修改、修正 strm）/ 完全（再加删 strm、按网盘重建 strm、删追更、整理冲突选「删掉 / 覆盖」）。只读令牌根本看不到会改网盘的工具。账号 cookie、设置、备份这些永远不对令牌开放。
- **工具集**：同步、转存与云下载、整理、追更、strm 管理五组，可以只开需要的几组，工具少了本地小模型选得更准。选「全部」记下的是当时的那几组：以后的版本加了新的一组，要回来勾上才能用——手建的令牌点「修改」，网页客户端在「已连接的客户端」里点「改工具组」，改完立即生效。
