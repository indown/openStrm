...
- **权限档**：只读（查任务、同步进度和记录、搜资源、网盘目录、分享内容、云下载列表、整理清单、追更、strm 体检）/ 日常（再加开始 / 取消同步、转存分享、添加云下载、整理的预览 / 改清单 / 执行 / 撤销、追更的检查和修改、修正 strm）/ 完全（再加删 strm、按网盘重建 strm、删追更、整理冲突选「删掉 / 覆盖」）。只读令牌根本看不到会改网盘的工具。账号 cookie、设置、备份这些永远不对令牌开放。
- **工具集**：同步、搜资源 / 转存与云下载、整理、追更、strm 管理五组，可以只开需要的几组，工具少了本地小模型选得更准。选「全部」记下的是当时的那几组：以后的版本加了新的一组，要回来勾上才能用——手建的令牌点「修改」，网页客户端在「已连接的客户端」里点「改工具组」，改完立即生效。
...
- **令牌只显示一次**，库里只存哈希；用不到了就在设置页撤销，怀疑泄露就「全部撤销」。改密码不会让令牌失效，改密码页上可以勾「同时撤销全部智能体令牌」。
...
在别的设备上批准（比如授权页开在电脑上，人拿着手机）：授权页上有一个配对码，到设置页「待批准」点「批准」，输入配对码和当前密码；在 Telegram 页打开「允许批准网页客户端的连接」后，把配对码发给机器人也行。要输配对码是因为授权码是发给「拿着授权页」的那个人的，而授权页谁都能打开、也能冒充 claude.ai 发起：输入你眼前授权页上的配对码，批的才一定是你自己这一条。不是你发起的请求直接拒绝（请求多了可以「全部拒绝」）。给的档位不会超过客户端自己要的；已连接的客户端在设置页里随时可以断开。
