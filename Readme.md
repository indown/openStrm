...
- **直接发链接**：磁力 / ed2k / http 链接 → 选下到哪个任务目录（可逐层进入子文件夹，比如 `tv/某剧/Season 1`），交给 115 云下载，下完自动生成 strm；配置了「复制到 OpenList」的话还会多一个「115 默认目录，下完让 OpenList 复制走」按钮；115 分享链接 → 看一眼内容，选转存到哪个任务目录（同样可进子文件夹）并触发同步。
