...
让 AI 客户端通过 [MCP](https://modelcontextprotocol.io) 调用 OpenStrm：说一句「把这个分享转存到电视剧任务」「电影任务跑一下同步，跑完告诉我失败了哪些」，由它去调工具。

**开启**：设置页「智能体接入」一节，打开开关并保存；再「新建令牌」（要再输一次登录密码），每个客户端单独建一个。

- **权限档**：只读（查任务、同步进度和记录、网盘目录、分享内容、云下载列表）/ 日常（再加开始 / 取消同步、转存分享、添加云下载）/ 完全。只读令牌根本看不到会改网盘的工具。账号 cookie、设置、备份这些永远不对令牌开放。
- **工具集**：可以只开「同步」或「转存与云下载」一组，工具少了本地小模型选得更准。选「全部」记下的是现在的这几组：以后的版本加了新的一组，要回来勾上才能用。
