...
- 智能体接入（MCP）：Claude Code、Codex、Open WebUI 这类 AI 客户端用令牌连上来，claude.ai、ChatGPT 网页从公网走 OAuth 连上来，能查任务和同步进度、开始同步、搜资源、看分享、转存、添加云下载（可顺带复制到 OpenList，复制队列能看、失败的能重试），替你核对整理清单（认不准的去 TMDB 找、冲突选办法）、你点头后执行或撤销，管理追更，查找、体检、修正 strm；分权限档，每次调用都有记录
...
接上自己部署的 [PanSou](https://github.com/fish2018/pansou)（网盘资源搜索服务，怎么部署、怎么配频道和插件见它的项目主页），就能按片名搜网盘分享和磁力：侧栏「资源搜索」页、顶栏输入框、⌘K、Telegram（`/s 片名`）和智能体都能搜。搜到的 115 / 夸克分享一键转存，磁力交给 115 云下载。

在设置页「资源搜索」一节填 PanSou 的地址（它开了登录的再填用户名和密码），点「检查连接」看看通没通；链接有效性检测、屏蔽词也在这一节。后端设了 `HTTP_PROXY` / `HTTPS_PROXY` 的话，把 PanSou 的主机名写进 `NO_PROXY`，不然请求会绕到代理上去。
...
- **权限档**：只读（查任务、同步进度和记录、搜资源、网盘目录、分享内容、云下载列表、复制到 OpenList 的队列、整理清单、追更、strm 体检）/ 日常（再加开始 / 取消同步、转存分享、添加云下载、重试失败的复制、整理的预览 / 改清单 / 执行 / 撤销、追更的检查和修改、修正 strm）/ 完全（再加删 strm、按网盘重建 strm、删追更、执行带删除项的整理）。整理清单里给冲突选「删掉 / 覆盖」日常档就能做（只是改清单）；清单里有删除项时，没有完全档的它执行不了，会请你在整理页点执行。只读令牌根本看不到会改网盘的工具。账号 cookie、设置、备份这些永远不对令牌开放。
- **工具集**：同步、搜资源 / 转存与云下载（复制到 OpenList 的队列也在这一组）、整理、追更、strm 管理五组，可以只开需要的几组，工具少了本地小模型选得更准。选「全部」记下的是当时的那几组：以后的版本加了新的一组，要回来勾上才能用——手建的令牌点「修改」，网页客户端在「已连接的客户端」里点「改权限」，改完立即生效。
...
- **复制到 OpenList**：转存、云下载可以要这一次也复制（任务没开复制也行），不说就按任务上的开关；任务开着复制的没法这一次不复制（和界面一样，网盘监控也会把新文件交给复制）。云下载指定复制到哪，只能选任务上或设置页填的目标目录、以及它们下面的目录。结果里会说排没排上、复制到哪、复制完删不删网盘上的源文件；任务开着「复制后删源」的，工具说明要求它事先告诉你。复制队列它能看、失败的能重试；「不跟了」只能在云下载页点。
