...
2. 新建同步任务时开启 **302 开关**。前缀填本地挂载路径（如 `/mnt/115`，会自动拼上 /账号名）或 OpenList 的 `/d/` 地址（如 `http://192.168.1.10:5244/d/115`，不拼账号名）都可以：换到 115 直链就 302，换不到时回给 Emby 按 strm 里的地址播放。用 OpenList 地址时 `/d/` 要能无签名访问，且挂的是同一个 115 账号
