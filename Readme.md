...
- 支持自定义前缀（方便配合媒体服务器使用）
- 基于115目录树生成
- 支持夸克网盘账号：同步生成 strm、随片下载字幕 / nfo、分享转存、追更、网盘监控、strm 校验与重新生成，和 115 用同一套页面。夸克没有整树导出，一次同步要逐个目录翻页，大库别把定时设太密；夸克直链取文件必须带 Cookie，所以不支持 302
...
- 网盘监控：115 走生活事件流实时增量；夸克没有事件流，靠定时把任务目录整棵列一遍和上一轮快照对比（最短 5 分钟一轮，大库等于高频同步），一轮里消失超过三成先不删本地文件，下一轮还是这样才删
- 115 云下载：磁力 / ed2k / http 链接交给 115 云端下载，下到任务目录后自动生成 strm
- 整理与规范化命名：识别网盘里的影视文件（TMDB），按 `作品 (年份) [tmdbid=…]/Season 01/作品 - S01E01.ext` 这样的媒体服务器规范改名并归到标准目录，本地 strm 跟着挪；先预览再执行，做过的能撤销；多版本电影、字幕 / nfo 跟随、识别词、二级分类都有；转存 / 追更 / 云下载 / 监控新增的文件可按任务设成自动整理或只生成待确认清单
- 支持账号级限流和重试逻辑
- 轻量，无额外依赖，易于二次开发
...
2. 新建同步任务时开启 **302 开关**。前缀填本地挂载路径（如 `/mnt/115`，会自动拼上 /账号名）或 OpenList 的 `/d/` 地址（如 `http://192.168.1.10:5244/d/115`，不拼账号名）都可以：换到 115 直链就 302，换不到时回给 Emby 按 strm 里的地址播放。用 OpenList 地址时 `/d/` 要能无签名访问（全局「签名所有」和该存储的「启用签名」都关掉）、挂的是同一个 115 账号；关掉签名后知道路径的人都能直接下载，所以 OpenList 只需对 Emby 可达，不要开在公网上
...
- **通知**：任务完成 / 失败（含定时触发的启动失败，带原因）、云下载完成、追更转存结果、网盘账号 cookie 失效或被封控（同一原因一小时只提醒一次）。每类可单独关掉，任务开始默认不发。
