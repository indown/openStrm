...
- 分享转存与追更：顶栏贴 115 / 夸克分享链接就能看内容、转存到同一网盘的任务目录并生成 strm，勾上「追更」后定时检查分享里的新增并自动转存（夸克分享条目没有 sha1，改名 / 换文件靠分享内 id 识别）
- 网盘监控：115 走生活事件流实时增量；夸克没有事件流，靠定时把任务目录整棵列一遍和上一轮快照对比（最短 5 分钟一轮，大库等于高频同步），一轮里消失超过三成不删本地文件
