 ...
 *
 * 用例之间有顺序依赖（先登录拿 token，再改密）；node:test 在同一个文件里按注册顺序串行跑。
 *
 *   CONFIG_DIR=... DATA_DIR=... pnpm test:file src/routes/auth/auth.itest.ts
 */
...
test("改密码接口自身不被拦截，且校验当前密码", async () => {
...
test("提供正确的当前密码后修改成功，标记随之翻转", async () => {
...
test("升级前的明文口令仍能登录，并就地转成哈希", async () => {
...
test("存量库里的明文默认口令同样会被要求改密", async () => {
