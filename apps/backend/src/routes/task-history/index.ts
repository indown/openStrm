...
export default async function (fastify: FastifyInstance) {
  ...
  fastify.get("/api/taskHistory", { preHandler: [fastify.authenticate], config: { agentScope: "read", agentToolset: "sync" } }, async (request) => {
    const { taskId } = parse(listQuerySchema, request.query, "query");
    return taskId ? getTaskHistory(taskId) : getAllTaskHistory();
  });

  fastify.get("/api/taskHistory/:executionId", { preHandler: [fastify.authenticate], config: { agentScope: "read", agentToolset: "sync" } }, async (request) => {
