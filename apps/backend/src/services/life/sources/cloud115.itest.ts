/**
 * 115 变更源 + 监控循环的离线集成测试：拉取和「开生活事件开关」都换成本地桩，不打 115。
 * 走的是真实的 Cloud115Provider（注册表按账号类型构造），覆盖门禁、405 降级、路径还原这些 115 专属行为；
 * 循环本身的通用行为（游标推进、隔离、取消）在 ../monitor.itest.ts 里用 FakeDrive 测。
 *
 * 循环的第一轮在 startLifeMonitor 返回后立刻跑，第二轮要等 intervalSeconds（下限 5s），
 * 所以用例只看第一轮的结果，然后 stop（stop 会立刻叫醒等待中的循环）。
 *
 *   CONFIG_DIR=... DATA_DIR=... pnpm test:file src/services/life/sources/cloud115.itest.ts
 */
...
import { setCloud115SourceDeps } from "./cloud115.js";
...
test("未启动时状态就列出配置里的每个账号，游标显示上次停的位置", () => {
  ...
  assert.deepEqual(statusOf("B")?.cursor, { time: 5, id: "77" }, "老格式 {fromTime, fromId} 也认");
});
...
test("两个账号各跑一条循环：事件按账号落库，游标各存各的", async () => {
  ...
  try {
    ...
    assert.equal(readKv<{ id: string }>(KEY.lifeCursor("A"))?.id, "1001");
    assert.equal(readKv<{ id: string }>(KEY.lifeCursor("B"))?.id, "2001");
    assert.equal(rows.find((e) => e.id === "1001")?.path, "/a-dir/a.mkv", "事件落库时已经还原成绝对路径");
    assert.equal(rows.find((e) => e.id === "1001")?.kind, "create");
  } finally {
    await stopLifeMonitor();
  }
  const s = getLifeMonitorStatus();
...
test("一个账号起不来不影响另一个：结果标成部分启动，状态里能看到原因和上次游标", async () => {
  ...
  try {
    ...
    assert.match(r.message, /^网盘监控已启动（账号 A）；B 未启动：115 生活事件不可用：请重新登录，请检查 cookie$/);
    ...
    assert.deepEqual(statusOf("B")?.cursor, { time: 5, id: "77" }, "起不来的账号照样显示库里的游标");
  } finally {
    await stopLifeMonitor();
  }
});
...
test("配置里的账号不存在 → 该账号启动失败，其它照常", async () => {
  ...
  try {
    assert.equal(r.ok, true, r.message);
    assert.match(r.message, /ghost 未启动：账户页里没有这个账号/);
...
test("last 模式从该账号上次的游标继续，并沿用它自己的降级窗口", async () => {
  writeKv(KEY.lifeCursor("B"), { time: 123, id: "999" });
  ...
  try {
    assert.equal(r.ok, true, r.message);
    assert.deepEqual(statusOf("B")?.cursor, { time: 123, id: "999" });
    assert.equal(statusOf("B")?.source, "webapi");
    assert.equal(statusOf("A")?.source, "proapi");
    ...
    assert.notDeepEqual(statusOf("A")?.cursor, { time: 123, id: "999" }, "A 没有存过游标，从现在开始");
  } finally {
    await stopLifeMonitor();
  }
});
...
test("启动刚发起就停止：启动按取消收场，不会留下循环", async () => {
  ...
  assert.equal(stopped.message, "网盘监控已停止");
...
test("115 事件还原成路径：按时间正序处理，移动事件的旧路径来自 path_cache", async () => {
  configure({ accounts: ["A"], pullMode: "latest", intervalSeconds: 5 });
  // 9101 先建后移：移动事件的旧路径来自 create 时写进缓存的那条（父目录都预置在缓存里，不会去打祖先链接口）
  rememberPath({ fileId: "11", parentId: "10", name: "sub", path: "/a-dir/sub", isDir: true, accountName: "A" });
  eventsFor = {
    A: [
      ev({ id: "3003", type: 6, file_id: "9101", parent_id: "11", file_name: "x.mkv", update_time: 1_900_000_003 }),
      ev({ id: "3001", type: 2, file_id: "9101", parent_id: "10", file_name: "x.mkv", update_time: 1_900_000_001 }),
    ],
  };
  const r = await startLifeMonitor();
  try {
    assert.equal(r.ok, true, r.message);
    await waitFor(() => (statusOf("A")?.stats.rounds ?? 0) >= 1 && (statusOf("A")?.stats.skipped ?? 0) === 2, "两条都处理完（没有任务，跳过）");
    const rows = listRecentLifeEvents(10);
    const byId = (id: string) => rows.find((e) => e.id === id)!;
    assert.equal(byId("3001").path, "/a-dir/x.mkv");
    assert.equal(byId("3001").kind, "create");
    assert.equal(byId("3003").kind, "move");
    assert.equal(byId("3003").oldPath, "/a-dir/x.mkv", "移动事件的旧路径来自缓存");
    assert.equal(byId("3003").path, "/a-dir/sub/x.mkv");
    assert.equal(byId("3003").type, 6, "115 事件保留自己的 behavior type");
    assert.equal(readKv<{ id: string }>(KEY.lifeCursor("A"))?.id, "3003", "游标推进到最新（时间正序处理）");
  } finally {
    await stopLifeMonitor();
  }
});
