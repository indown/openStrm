...
test("同 id 换路径：父目录不变是 rename，变了是 move；目录整体搬走时子项不单报", () => {
  ...
  assert.deepEqual(brief(r), ["create misc", "rename S1 -> Season 1", "move x.mkv -> misc/x.mkv"]);
  // misc 是新目录：它本身 create，里面搬进来的 x.mkv 仍要报 move（本地要挪而不是重新生成）
...
test("同名替换：分享者删了再传同名文件 → 新 id，报一条 remove 一条 create", () => {
  const prev = [f("a.mkv", "f1")];
  const curr = [f("a.mkv", "f9")];
  assert.deepEqual(brief(diffSnapshot(prev, curr)), ["create a.mkv", "remove a.mkv"]);
});

test("输出顺序：新增在前、搬动其次、删除最后；同组内浅的在前", () => {
  const prev = [d("A", "d1"), f("A/x.mkv", "f1"), f("old.mkv", "f2")];
  const curr = [d("B", "d1"), f("B/x.mkv", "f1"), d("N", "d3"), f("N/deep.mkv", "f4"), f("n.mkv", "f5")];
  assert.deepEqual(brief(diffSnapshot(prev, curr)), ["create N", "create n.mkv", "rename A -> B", "remove old.mkv"]);
});
